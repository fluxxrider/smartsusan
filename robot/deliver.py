...
MODEL = os.path.expanduser("~/snack-rotator/runs/marker_v4/best.pt")  # v3 fallback: runs/marker_v3/weights/best.pt
SNACK_MODEL = os.path.expanduser("~/snack-rotator/snacks_best.pt")
...
def load_model(snacks=False):
    """Returns a list of (yolo_model, allowed_class_set)."""
    from ultralytics import YOLO
    if snacks:
        global TAPES
        TAPES[:] = SNACKS   # snack classes replace tape classes everywhere
        unified = os.path.expanduser("~/snack-rotator/runs/snacks_unified.pt")
        if os.path.exists(unified):
            return [(YOLO(unified), None)]          # one model, all 5 classes, trained on this rig
        return [(YOLO(SNACK_MODEL), set(SNACKS)), (YOLO(MODEL), {"x"})]
    return [(YOLO(MODEL), None)]

CLASS_CONF = {"x": 0.20, "black": 0.30, "red": 0.35, "blue": 0.35, "silver": 0.30,
              "bar": 0.45, "bear": 0.45, "juice": 0.45, "welchs": 0.45}
...
def detect(models, img):
    """Return {name: (cx, cy, conf)}: best per class, cross-class overlaps suppressed.
    `models` is a list of (yolo, allowed_class_set|None) - detections are merged."""
    if not isinstance(models, list):
        models = [(models, None)]
    cands = []
    for model, allowed in models:
...
class EV3Board:
    ...
    def goto(self, deg):
        self.m.start_move_to(round(deg), speed=15, brake=True)
        while self.m.busy:
            time.sleep(0.05)
        time.sleep(0.4)          # let the platter and snacks settle
        return round(deg)
...
if __name__ == "__main__":
    ...
    p.add_argument("snack", choices=TAPES + SNACKS)
    p.add_argument("--snacks", action="store_true", help="real-snack mode (dual model)")
    ...
    if args.snack in SNACKS:
        args.snacks = True
    model = load_model(snacks=args.snacks)
    if args.snacks and args.snack in ("black", "red", "blue", "silver"):
        sys.exit("in --snacks mode request one of: " + " ".join(SNACKS))
    if args.image:
